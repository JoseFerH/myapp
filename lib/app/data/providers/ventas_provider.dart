...
class VentasProvider {
  ...
  Future<List<VentaModel>> obtenerVentas() async {
    try {
      final QuerySnapshot snapshot =
          await _dbProvider.ventasRef.orderBy('fecha', descending: true).get();

      return snapshot.docs
          .map(
            (doc) => VentaModel.fromFirestore(
  ...
  Future<VentaModel> obtenerVentaPorId(String id) async {
    try {
      ...
      return VentaModel.fromFirestore(
        doc as DocumentSnapshot<Map<String, dynamic>>,
      );
    } catch (e) {
  ...
  Future<String> crearVenta(VentaModel venta) async {
    try {
      ...
      if (venta.estado == EstadoVenta.completada) {
        await _clientesProvider.actualizarTotalGastado(
          venta.clienteId,
          venta.total,
        );
      }
  ...
  Future<void> actualizarVenta(VentaModel venta) async {
    try {
      ...
      final DocumentSnapshot doc =
          await _dbProvider.ventasRef.doc(venta.id).get();

      if (!doc.exists) {
        throw Exception('Venta no encontrada');
      }

      VentaModel ventaAnterior = VentaModel.fromFirestore(
        doc as DocumentSnapshot<Map<String, dynamic>>,
      );

      // Actualizar la venta
      await _dbProvider.ventasRef.doc(venta.id).update(venta.toMap());
      ...
      if (ventaAnterior.estado == EstadoVenta.completada &&
        ...
        await _clientesProvider.actualizarTotalGastado(
          venta.clienteId,
          venta.total - ventaAnterior.total,
        );
      }
    } catch (e) {
  ...
  Future<void> eliminarVenta(String id) async {
    try {
      ...
      VentaModel venta = VentaModel.fromFirestore(
        doc as DocumentSnapshot<Map<String, dynamic>>,
      );
  ...
  Future<void> cambiarEstadoVenta(String id, EstadoVenta nuevoEstado) async {
    try {
      ...
      await _dbProvider.ventasRef.doc(id).update({
    ...
    try {
      ...
      ventasPorDia.forEach((fecha, total) {
        ...
        String diaSemana = DateFormat(
          'EEE',
        ).format(fechaDt); // Abreviación del día

        datos.add({'fecha': fecha, 'dia': diaSemana, 'total': total});
      });
