...
class EstadisticasService extends GetxService {
  ...
  final RxList<Map<String, dynamic>> datosGraficoVentas = <Map<String, dynamic>>[].obs;
  final RxList<ClienteModel> topClientes = <ClienteModel>[].obs;
  final RxList<Map<String, dynamic>> productosPopulares = <Map<String, dynamic>>[].obs;
  ...
  Future<void> cargarResumenVentas() async {
    try {
      ...
      Map<String, dynamic> hoy = await _ventasProvider.obtenerEstadisticasVentas('hoy');
      
      // Obtener estadísticas de la semana
      Map<String, dynamic> semana = await _ventasProvider.obtenerEstadisticasVentas('semana');
      
      // Obtener estadísticas del mes
      Map<String, dynamic> mes = await _ventasProvider.obtenerEstadisticasVentas('mes');
  ...
  Future<void> cargarDatosGrafico() async {
    try {
      List<Map<String, dynamic>> datos = await _ventasProvider.obtenerDatosGraficoVentas();
  ...
  Future<void> cargarTopClientes() async {
    try {
      List<ClienteModel> clientes = await _clientesProvider.obtenerTopClientes(5);
  ...
  Future<void> cargarProductosPopulares() async {
    try {
      ...
      List<VentaModel> ventas = await _ventasProvider.obtenerVentasPorFecha(hace30Dias, ahora);
      ...
      for (var venta in ventas) {
        for (var item in venta.items) {
          ...
          final clave = '${item.tamano.toString()}_${item.hojaId}_${item.laminadoId}';
      ...
      List<Map<String, dynamic>> listaProductos = conteoProductos.values.toList();
      
      // Ordenar por cantidad vendida
      listaProductos.sort((a, b) => (b['cantidad'] as int).compareTo(a['cantidad'] as int));
  ...
  Future<void> cargarMaterialesBajoStock() async {
    try {
      List<MaterialModel> materiales = await _materialesProvider.obtenerMaterialesBajoStock();
  ...
  double obtenerPromedioVentaPeriodo(String periodo) {
    if (resumenVentas.containsKey(periodo)) {
      return resumenVentas[periodo]['promedioVenta'] ?? 0.0;
    }
    return 0.0;
  }
  
  // Obtener porcentaje de variación entre períodos
  double obtenerVariacionPorcentual(String periodoActual, String periodoAnterior) {
