...
class VentaService extends GetxService {
  ...
  Future<List<VentaModel>> cargarVentasPorFecha(DateTime fechaInicio, DateTime fechaFin) async {
    try {
      cargando.value = true;
      error.value = '';
      
      List<VentaModel> listaVentas = await _ventasProvider.obtenerVentasPorFecha(fechaInicio, fechaFin);
      return listaVentas;
    } catch (e) {
  ...
  Future<List<VentaModel>> cargarVentasPorCliente(String clienteId) async {
    try {
      ...
      List<VentaModel> listaVentas = await _ventasProvider.obtenerVentasPorCliente(clienteId);
      return listaVentas;
    } catch (e) {
  ...
  Future<bool> actualizarVenta(VentaModel venta) async {
    try {
      ...
      VentaModel? ventaAnterior = await _ventasProvider.obtenerVentaPorId(venta.id);
      ...
      if (ventaAnterior.estado != EstadoVenta.completada && venta.estado == EstadoVenta.completada) {
        ...
        await _reducirStockMateriales(venta.items);
      } else if (ventaAnterior.estado == EstadoVenta.completada && venta.estado != EstadoVenta.completada) {
  ...
  Future<bool> cambiarEstadoVenta(String id, EstadoVenta nuevoEstado) async {
    try {
      ...
      if (estadoAnterior != EstadoVenta.completada && nuevoEstado == EstadoVenta.completada) {
        // Si cambia a completada, reducir stock
        await _reducirStockMateriales(venta.items);
      } else if (estadoAnterior == EstadoVenta.completada && nuevoEstado != EstadoVenta.completada) {
        // Si cambia de completada a otro estado, aumentar stock
        await _aumentarStockMateriales(venta.items);
      }
