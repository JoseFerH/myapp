...
class InventarioView extends GetView<RegistrosController> {
  ...
  void _mostrarFormularioMaterial(BuildContext context, [MaterialModel? material]) {
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          _buildInfoRow('Tipo:', _obtenerTipoTexto(material.tipo)),
                          const SizedBox(height: 8),
                          _buildInfoRow('Precio Unitario:', 'Q${material.precioUnitario.toStringAsFixed(2)}'),
                          const SizedBox(height: 8),
                          _buildInfoRow('Stock Disponible:', '${material.cantidadDisponible} unidades'),
                          const SizedBox(height: 8),
                          _buildInfoRow('Stock Mínimo:', '${material.cantidadMinima} unidades'),
                          const SizedBox(height: 8),
                          _buildInfoRow('Fecha de Compra:', DateFormat('dd/MM/yyyy').format(material.fechaCompra)),
                          
                          // Información específica según tipo de material
                          if (material is HojaModel) ...[
                            const SizedBox(height: 8),
                            _buildInfoRow('Tipo de Hoja:', _obtenerTipoHojaTexto(material.tipoHoja)),
                            const SizedBox(height: 8),
                            _buildInfoRow('Tamaño:', material.tamano),
                          ] else if (material is LaminadoModel) ...[
                            const SizedBox(height: 8),
                            _buildInfoRow('Tipo de Laminado:', _obtenerTipoLaminadoTexto(material.tipoLaminado)),
                            const SizedBox(height: 8),
                      ...
                      children: [
                        Expanded(
                          child: CupertinoButton(
                            ...
                            onPressed: () => _mostrarDialogoAgregarStock(context, material),
                          ),
                        ),
                        ...
                        Expanded(
                          child: CupertinoButton(
                            ...
                            onPressed: () => _mostrarDialogoReducirStock(context, material),
                          ),
                        ),
                      ],
                    ),
                ...
                children: [
                  ...
                  Expanded(
                    child: CupertinoButton(
                      ...
                      onPressed: () => _confirmarEliminarMaterial(context, material),
                    ),
                  ),
                ],
              ),
  ...
  void _mostrarDialogoAgregarStock(BuildContext context, MaterialModel material) {
            ...
            onPressed: () {
              if (cantidadController.text.isNotEmpty) {
                try {
                  ...
                  if (cantidad > 0) {
                    controller.aumentarStock(material.id, cantidad).then((success) {
                      if (success) {
  ...
  void _mostrarDialogoReducirStock(BuildContext context, MaterialModel material) {
    final cantidadController = TextEditingController();
                ...
                try {
                  ...
                  if (cantidad > 0) {
                    if (cantidad <= material.cantidadDisponible) {
                      controller.reducirStock(material.id, cantidad).then((success) {
                        if (success) {
                          Navigator.pop(context); // Cerrar diálogo
                          Navigator.pop(context); // Cerrar detalle
                          Get.snackbar(
  ...
  void _confirmarEliminarMaterial(BuildContext context, MaterialModel material) {
    showCupertinoDialog(
      context: context,
      builder: (context) => CupertinoAlertDialog(
